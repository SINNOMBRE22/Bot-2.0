...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  const datas = global;
  let idioma = (datas.db?.data?.users?.[m.sender]?.language || global.defaultLenguaje || 'es');
  let _translate;
  try {
    _translate = JSON.parse(readFileSync(`./src/languages/${idioma}.json`));
  } catch {
    _translate = { plugins: { audio_efectos: { texto1: 'Necesitas enviar un audio con el comando' } } };
  }
  ...
  try {
    ...
    if (/audio/.test(mime)) {
      ...
      try {
        ...
        try {
          if (media && existsSync(media)) unlinkSync(media);
...
handler.help = ['bass', 'blown', 'deep', 'earrape', 'fast', 'fat', 'nightcore', 'reverse', 'robot', 'slow', 'tupai', 'echo', 'chorus', 'flanger', 'vibrato', 'tremolo', 'phaser', 'compressor', 'distortion', 'underwater', 'telephone', 'radio', 'cave', 'whisper', 'demon'];
handler.tags = ['effects'];
handler.command = /^(bass|blown|deep|earrape|fas?t|fat|nightcore|reverse|robot|slow|tupai|squirrel|chipmunk|echo|chorus|flanger|vibrato|tremolo|phaser|compressor|distortion|underwater|telephone|radio|cave|whisper|demon)$/i;
