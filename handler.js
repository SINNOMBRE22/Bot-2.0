...
export async function handler(chatUpdate) {
  ...
  /* ------------------------------------------------*/
    ...
    // 🔧 Obtenemos un mensaje "citado" seguro
    let quotedMessage = m.quoted?.message ? smsg(this, m.quoted) : null;

    // Si no hay cita real pero existe contexto, la cargamos
    if (!quotedMessage && m.message?.extendedTextMessage?.contextInfo?.stanzaId) {
        const ctx = m.message.extendedTextMessage.contextInfo;
        try {
            const qmsg = await this.loadMessage(ctx.remoteJid, ctx.stanzaId);
            if (qmsg?.message) {
                quotedMessage = smsg(this, qmsg);
            }
        } catch (e) {
            console.log('⚠️ No se pudo recuperar el mensaje citado:', e);
        }
    }

    // Si todavía no hay cita, usamos el mismo mensaje como fallback
    if (!quotedMessage) {
        quotedMessage = {
            message: m.message,
            sender: m.fromMe ? this.user.id : m.sender,
            fromMe: m.fromMe || true
        };
    }

    // Guardamos el mensaje citado seguro en otra propiedad
    m.quotedMessage = quotedMessage;
